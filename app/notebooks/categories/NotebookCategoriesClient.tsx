...
type NotebookCategoriesClientProps = {
  initialCategory: NotebookCategory;
};

function NotebookCategoriesClient({ initialCategory }: NotebookCategoriesClientProps) {
  ...
  useEffect(() => {
    setCurrentNotebookCategory(category);
            ...
            {categories.map(({ id: categoryId, title }) => {
              if (title !== category)
                return (
                  <Link
                    key={categoryId}
                    href={`/notebooks/categories?category=${categoryId}`}
...
export default NotebookCategoriesClient;
